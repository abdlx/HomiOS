...
function InstalledAppsView({apps,catalogById,busy,actionStates,onDomains,onAction}:{apps:InstalledApp[];catalogById:Map<string,CatalogApp>;busy:boolean;actionStates:Record<string,AppActionState>;onDomains:(app:InstalledApp)=>void;onAction:(id:string,name:string,method?:string)=>void}){
  ...
  return <div><div className="mb-4"><h3 className="text-lg font-semibold text-slate-900 dark:text-white">{apps.length} installed {apps.length===1?'app':'apps'}</h3><p className="text-xs text-slate-400">Manage addresses and runtime controls from one place</p></div><div className="grid gap-4 lg:grid-cols-2 2xl:grid-cols-3">{apps.map((app)=><InstalledAppCard key={app.id} app={app} catalogApp={catalogById.get(app.catalogId)} busy={busy} actionState={actionStates[app.id]} onDomains={()=>onDomains(app)} onAction={onAction}/>)}</div></div>;
}
function InstalledAppCard({app,catalogApp,busy,actionState,onDomains,onAction}:{app:InstalledApp;catalogApp?:CatalogApp;busy:boolean;actionState?:AppActionState;onDomains:()=>void;onAction:(id:string,name:string,method?:string)=>void}){
