...
export default function DockerManagerApp({ onClose, initialAppId }: AppProps) {
  ...
  const deleteApp = async (appId: string) => {
    if (!confirm('Delete this resource? Containers are removed; named volumes are preserved. This cannot be undone.')) return;
  ...
  const ec = 'w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-2.5 text-sm text-gray-800 focus:border-[#007aff] focus:outline-none transition';
    ...
    <div className="h-full w-full flex bg-gray-50 select-none overflow-hidden font-sans relative">
      ...
      <div className={`${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 absolute md:static z-50 h-full md:h-auto transition-transform duration-300 ease-in-out flex flex-col justify-between w-[240px] md:w-[250px] bg-white md:border md:border-neutral-200/50 shadow-2xl md:shadow-sm md:m-3 md:rounded-[32px] p-4 pt-5`}>
        <div className="flex flex-col h-full">
          ...
          <div className="mb-2 px-2 flex justify-between items-center">
            ...
            <button onClick={() => { setProjectName(''); setShowProjectModal(true); }} className="text-gray-400 hover:text-blue-500 transition-colors"><Plus size={14} /></button>
          </div>
            ...
            {projects.map((p) => {
              ...
              return (
                <button key={p.id} onClick={() => { setSelectedProject(p); setSelectedApp(null); setIsSidebarOpen(false); }}
                  className={`w-full flex items-center space-x-2 px-2 py-1.5 rounded-md text-sm text-left transition-colors font-medium ${isActive ? 'bg-blue-600/10 text-blue-600 font-bold' : 'text-gray-600 hover:bg-neutral-200/50 hover:text-gray-900'}`}>
                  <Layers size={16} className={`flex-shrink-0 ${isActive ? 'text-blue-600' : 'text-gray-400'}`} />
                  <span className="truncate flex-1">{p.name}</span>
                </button>
              );
            })}
            {projects.length === 0 && <div className="text-center py-6 text-xs text-gray-400 italic">No projects found.</div>}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-gray-50 md:pt-3 md:pr-3 md:pb-3 w-full">
        {selectedProject ? (
          selectedApp ? (
            <div className="flex flex-col h-full bg-white md:rounded-[32px] md:border border-neutral-200/50 shadow-sm overflow-hidden transform-gpu">
              <div className="px-4 md:px-8 py-6 border-b border-neutral-100 flex justify-between items-start md:rounded-t-[32px] bg-white">
                <div className="flex gap-3">
                  <button onClick={() => setIsSidebarOpen(true)} className="md:hidden mt-1 text-gray-500"><Menu size={20} /></button>
                  <div>
                    ...
                    <h1 className="text-xl md:text-2xl font-bold text-gray-900 flex flex-wrap items-center gap-2">{selectedApp.name}{statusPill(selectedApp.status)}</h1>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 justify-end flex-shrink-0">
                  {selectedApp.status === 'running' ? (
                    <button onClick={() => lifecycle(selectedApp.id, 'stop')} className="px-3 py-2 bg-white hover:bg-neutral-50 text-gray-700 text-sm font-medium rounded-[10px] border border-neutral-200 flex items-center shadow-sm"><Square size={14} className="mr-1.5 text-red-500" /> Stop</button>
                  ) : (
                    <button onClick={() => lifecycle(selectedApp.id, 'start')} disabled={selectedApp.status === 'deploying'} className="px-3 py-2 bg-white hover:bg-neutral-50 text-gray-700 text-sm font-medium rounded-[10px] border border-neutral-200 flex items-center shadow-sm disabled:opacity-40"><Play size={14} className="mr-1.5 text-green-600" /> Start</button>
                  )}
                  <button onClick={() => lifecycle(selectedApp.id, 'restart')} disabled={selectedApp.status === 'deploying'} className="px-3 py-2 bg-white hover:bg-neutral-50 text-gray-700 text-sm font-medium rounded-[10px] border border-neutral-200 flex items-center shadow-sm disabled:opacity-40"><RotateCcw size={14} className="mr-1.5" /> Restart</button>
                  <button onClick={() => lifecycle(selectedApp.id, 'rollback')} disabled={selectedApp.status === 'deploying'} className="px-3 py-2 bg-white hover:bg-neutral-50 text-gray-700 text-sm font-medium rounded-[10px] border border-neutral-200 flex items-center shadow-sm disabled:opacity-40" title="Roll back to previous deployment"><History size={14} className="mr-1.5" /> Rollback</button>
              ...
              <div className="px-4 md:px-8 border-b border-neutral-100 flex space-x-6 overflow-x-auto hide-scrollbar">
                {['config', 'metrics', 'logs'].map((tab) => (
                  <button key={tab} onClick={() => setActiveTab(tab as any)} className={`py-3 text-sm font-medium transition-colors border-b-2 ${activeTab === tab ? 'border-[#007aff] text-[#007aff]' : 'border-transparent text-gray-500 hover:text-gray-800'}`}>{tab.charAt(0).toUpperCase() + tab.slice(1)}</button>
                ))}
              </div>
                  ...
                  <div className="max-w-3xl space-y-8">
                      ...
                      <div className="flex justify-between items-center mb-5">
                        ...
                        {editMode ? (
                          <div className="flex gap-2">
                            <button onClick={() => setEditMode(false)} className="px-3 py-1.5 text-sm font-medium text-gray-600 border border-neutral-200 rounded-[10px] hover:bg-neutral-50">Cancel</button>
                          ...
                          <button onClick={() => { setEditApp({ ...selectedApp }); setEditMode(true); }} className="px-3 py-1.5 text-sm font-medium text-gray-700 border border-neutral-200 rounded-[10px] hover:bg-neutral-50 flex items-center"><Pencil size={14} className="mr-1.5" /> Edit</button>
                        )}
                      </div>
                      <div className="space-y-4">
                        <div>
                          <label className="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-1.5">Webhook (CI/CD Auto-Deploy · secret-signed)</label>
                          <div className="flex">
                            <input type="text" readOnly value={webhookUrl} className="flex-1 bg-neutral-50 border border-neutral-200 rounded-l-[10px] p-2.5 text-sm text-gray-800 font-mono focus:outline-none truncate" />
                            <button onClick={() => { navigator.clipboard.writeText(webhookUrl); setCopied(true); setTimeout(() => setCopied(false), 1500); }} className="px-4 bg-gray-200 hover:bg-gray-300 text-gray-700 font-bold text-xs rounded-r-[10px] flex items-center">{copied ? <Check size={14} /> : <Copy size={14} />}</button>
                          </div>
                        </div>
                        {field('Name', 'name')}
                    ...
                    <section className="bg-white p-6 rounded-[20px] border border-neutral-200/60 shadow-sm">
                      <div className="flex justify-between items-center mb-4">
                        <div>
                          <h3 className="text-lg font-bold text-gray-900 flex items-center"><Archive size={18} className="mr-2 text-indigo-500" /> Volume Backups</h3>
                          <p className="text-xs text-gray-500 mt-0.5">Snapshot named volumes to compressed archives on the host.</p>
                        </div>
                      ...
                      {backups.length === 0 ? (
                        ...
                        <div className="space-y-2">
                          {backups.map((b) => (
                            <div key={b.file} className="flex justify-between items-center bg-neutral-50 border border-neutral-200 rounded-[10px] px-3 py-2 text-sm">
                              <span className="font-mono text-gray-700 truncate mr-3" title={b.file}>{b.file}</span>
                              <span className="text-gray-400 flex-shrink-0 text-xs">{(b.size / 1024 / 1024).toFixed(1)} MB · {new Date(b.created).toLocaleString()}</span>
                            </div>
                          ))}
                        </div>
                      )}
                    ...
                    <section className="p-6 rounded-[20px] border border-red-200 bg-red-50">
                      ...
                      <p className="text-sm text-red-500/80 mb-4">Permanently delete this resource. Containers are removed; named volumes are preserved.</p>
                      <button onClick={() => deleteApp(selectedApp.id)} className="px-4 py-2 bg-white hover:bg-red-100 text-red-600 border border-red-200 text-sm font-medium rounded-[10px] shadow-sm flex items-center"><Trash size={14} className="mr-2" /> Delete Resource</button>
                    </section>
                  </div>
                )}
                {activeTab === 'metrics' && (
                  <div className="max-w-4xl space-y-6">
                    {selectedApp.status !== 'running' ? (
                      ...
                      <div className="text-center py-10 text-gray-500 flex items-center justify-center"><Activity size={16} className="mr-2 animate-pulse" /> Waiting for Docker stats…</div>
                    ) : (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {metrics.map((m, i) => (
                          <div key={i} className="bg-white p-5 rounded-[20px] border border-neutral-200/60 shadow-sm relative overflow-hidden">
                            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-blue-400 to-indigo-500" />
                            <h4 className="font-bold text-gray-900 truncate mb-4 pr-6" title={m.name}>{m.name}</h4>
                            <div className="grid grid-cols-2 gap-4">
                              <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">CPU</p><p className="text-xl font-bold text-blue-600">{m.cpu}</p></div>
                              <div><p className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-1">Memory</p><p className="text-sm font-bold text-indigo-600 truncate">{m.mem}</p></div>
                            </div>
                          </div>
                        ))}
                      </div>
                ...
                {activeTab === 'logs' && (
                  <div className="h-full flex flex-col bg-gray-900 rounded-[20px] shadow-inner overflow-hidden">
                    ...
                    <div className="flex-1 p-4 overflow-y-auto font-mono text-sm leading-relaxed text-green-400">
                      {liveLogs.length === 0 ? <div className="text-gray-500 italic">No active logs.</div> : liveLogs.map((log, i) => <div key={i} className="break-all whitespace-pre-wrap">{log}</div>)}
                      <div ref={logsEndRef} />
                    </div>
                  </div>
                )}
            ...
            <div className="flex flex-col h-full bg-white md:rounded-[32px] md:border border-neutral-200/50 shadow-sm overflow-hidden relative transform-gpu">
              <div className="px-4 md:px-8 py-6 md:py-8 border-b border-neutral-100 flex justify-between items-end bg-white/80 backdrop-blur-md sticky top-0 z-10 md:rounded-t-[32px]">
                <div className="flex gap-3">
                  <button onClick={() => setIsSidebarOpen(true)} className="md:hidden mt-2 text-gray-500"><Menu size={24} /></button>
                  <div>
                ...
                <div className="flex gap-2">
                  <button onClick={openStore} className="px-4 py-2 bg-white border border-neutral-200 hover:bg-neutral-50 text-gray-800 font-medium rounded-[10px] shadow-sm flex items-center text-sm"><Store size={16} className="mr-1.5 text-indigo-500" /> App Store</button>
              ...
              <div className="flex-1 overflow-y-auto p-4 md:p-8 bg-gray-50">
                {apps.length === 0 ? (
                  <div className="flex flex-col items-center justify-center h-full text-center max-w-md mx-auto">
                    <div className="w-16 h-16 bg-white border border-neutral-200 rounded-full flex items-center justify-center mb-4 shadow-sm"><LayoutGrid size={28} className="text-[#007aff]" /></div>
                    <h3 className="text-xl font-bold text-gray-900 mb-2">No resources found</h3>
                    <p className="text-sm text-gray-500 mb-6">Start by adding a Docker container, a database, a Git repo, or a Compose stack.</p>
                    <button onClick={() => { setNewApp({ ...BLANK_APP }); setShowWizard(true); }} className="px-5 py-2.5 bg-white border border-neutral-200 hover:bg-neutral-50 text-gray-800 font-bold rounded-[12px] shadow-sm">Add your first resource</button>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {apps.map((app) => (
                      <div key={app.id} onClick={() => setSelectedApp(app)} className="bg-white rounded-[20px] border border-neutral-200/60 p-5 cursor-pointer hover:border-[#007aff]/30 transition-all hover:shadow-md group flex flex-col h-44">
                        <div className="flex justify-between items-start mb-4">
                          <div className="flex items-center space-x-3">
                            <div className={`p-2.5 rounded-[12px] ${app.status === 'running' ? 'bg-green-100 text-green-600' : 'bg-gray-100 text-gray-500'}`}><Box size={20} /></div>
                            <div>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center bg-white md:rounded-[32px] md:border border-neutral-200/50 shadow-sm transform-gpu relative">
            <button onClick={() => setIsSidebarOpen(true)} className="md:hidden absolute top-4 left-4 text-gray-500"><Menu size={24} /></button>
            ...
            <p className="text-gray-500 font-medium">Select or create a project to manage deployments.</p>
          </div>
        )}
      </div>
      ...
      {showProjectModal && (
        <div className="fixed inset-0 z-[120] bg-black/20 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-[20px] shadow-2xl w-full max-w-sm border border-neutral-200/50 p-6">
            <h3 className="font-bold text-lg text-gray-900 mb-4">New Project</h3>
            ...
            <div className="flex justify-end gap-2 mt-5">
              <button onClick={() => setShowProjectModal(false)} className="px-4 py-2 text-sm font-medium text-gray-600 border border-neutral-200 rounded-[10px] hover:bg-neutral-50">Cancel</button>
      ...
      {showStore && (
        <div className="fixed inset-0 z-[120] bg-black/20 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-3xl border border-neutral-200/50 flex flex-col max-h-[90vh]">
            <div className="px-6 py-5 border-b border-neutral-100 flex justify-between items-center">
              <h3 className="font-bold text-xl text-gray-900 flex items-center"><Store size={20} className="mr-2 text-indigo-500" /> App Store</h3>
              <button onClick={() => setShowStore(false)} className="p-1.5 bg-gray-100 rounded-full text-gray-500 hover:bg-gray-200"><X size={16} /></button>
            </div>
              ...
              {!storeSel ? (
                catalog.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {catalog.map((e) => (
                      <button key={e.id} onClick={() => { setStoreSel(e); setStoreForm({ name: e.id, domains: '' }); }} className="text-left bg-white rounded-[16px] border border-neutral-200/60 p-4 hover:border-indigo-400/50 hover:shadow-md transition group">
                        <div className="flex items-center justify-between mb-2">
                          <span className="text-3xl">{e.icon}</span>
                          <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400 bg-gray-100 rounded-full px-2 py-0.5">{e.category}</span>
                        </div>
                        <h4 className="font-bold text-gray-900 group-hover:text-indigo-600 transition">{e.name}</h4>
                ...
                <div className="max-w-lg mx-auto">
                  ...
                  <div className="bg-white rounded-[16px] border border-neutral-200 p-6 shadow-sm">
                    <div className="flex items-center gap-3 mb-4"><span className="text-4xl">{storeSel.icon}</span><div><h4 className="font-bold text-lg text-gray-900">{storeSel.name}</h4><p className="text-xs text-gray-500">{storeSel.description}</p></div></div>
                    <div className="space-y-4">
                      <div><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Resource Name</label><input value={storeForm.name} onChange={(e) => setStoreForm({ ...storeForm, name: e.target.value })} className={ec} /></div>
                      {storeSel.needsDomain && (
                        <div><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Domain</label><input value={storeForm.domains} onChange={(e) => setStoreForm({ ...storeForm, domains: e.target.value })} placeholder="app.mydomain.com" className={ec} /><p className="text-[10px] text-gray-400 mt-1.5">Required — Traefik routes this domain to the app (HTTPS auto when ACME_EMAIL is set). Secrets are generated securely on the server.</p></div>
                      )}
                    </div>
      ...
      {showWizard && (
        <div className="fixed inset-0 z-50 bg-black/20 backdrop-blur-md flex items-center justify-center p-4">
          <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-2xl border border-neutral-200/50 flex flex-col max-h-[90vh]">
            <div className="px-6 py-5 border-b border-neutral-100 flex justify-between items-center rounded-t-[24px]">
              <h3 className="font-bold text-xl text-gray-900">Add New Resource</h3>
              <button onClick={() => setShowWizard(false)} className="p-1.5 bg-gray-100 rounded-full text-gray-500 hover:bg-gray-200"><X size={16} /></button>
            </div>
            <div className="p-6 overflow-y-auto custom-scrollbar bg-gray-50 rounded-b-[24px]">
              <div className="bg-white p-5 rounded-[16px] border border-neutral-200 mb-5 shadow-sm">
                <div className="mb-4">
                  ...
                  <input type="text" placeholder="my-frontend-app" value={newApp.name} onChange={(e) => setNewApp({ ...newApp, name: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Source Type</label>
                  <select value={newApp.build_pack} onChange={(e) => setNewApp({ ...newApp, build_pack: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none appearance-none cursor-pointer">
              ...
              <div className="bg-white p-5 rounded-[16px] border border-neutral-200 mb-5 shadow-sm">
                {newApp.build_pack === 'github' && (
                  <div className="grid grid-cols-4 gap-4 mb-5">
                    <div className="col-span-3"><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Git Repository URL</label><input type="text" placeholder="https://github.com/expressjs/express.git" value={newApp.git_repo} onChange={(e) => setNewApp({ ...newApp, git_repo: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none" /></div>
                    <div className="col-span-1"><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Branch</label><input type="text" placeholder="main" value={newApp.git_branch} onChange={(e) => setNewApp({ ...newApp, git_branch: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none" /></div>
                  </div>
                )}
                {newApp.build_pack === 'template' && (
                  <div className="mb-5">
                    ...
                    }} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] appearance-none cursor-pointer" defaultValue="">
                ...
                {newApp.build_pack === 'dockerimage' && (
                  <div className="grid grid-cols-4 gap-4 mb-5">
                    <div className="col-span-3"><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Image Name</label><input type="text" placeholder="nginx, redis, postgres" value={newApp.docker_image} onChange={(e) => setNewApp({ ...newApp, docker_image: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none" /></div>
                    <div className="col-span-1"><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Tag</label><input type="text" placeholder="latest" value={newApp.docker_image_tag} onChange={(e) => setNewApp({ ...newApp, docker_image_tag: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none" /></div>
                  </div>
                )}
                {newApp.build_pack === 'dockercompose' && (
                  <div className="mb-5"><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">docker-compose.yml content</label><textarea value={newApp.compose_content} onChange={(e) => setNewApp({ ...newApp, compose_content: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 font-mono h-40 focus:border-[#007aff] focus:outline-none" placeholder="services:\n  web:\n    image: nginx" /></div>
                )}

                <div className="mb-4"><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Domains (Automatic Proxy)</label><input type="text" placeholder="app.mydomain.com, api.local" value={newApp.domains} onChange={(e) => setNewApp({ ...newApp, domains: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none" /><p className="text-[10px] font-medium text-gray-400 mt-1.5">Comma-separated. Traefik routes these to the container (HTTPS auto when ACME_EMAIL is set).</p></div>
                <div className="mb-4"><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Port Mapping</label><input type="text" placeholder="8080:80, 5353:53/udp" value={newApp.ports} onChange={(e) => setNewApp({ ...newApp, ports: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none" /><p className="text-[10px] font-medium text-gray-400 mt-1.5">host:container[/proto], comma-separated.</p></div>
                <div className="mb-4"><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Persistent Volumes</label><input type="text" placeholder="my-volume:/var/lib/data" value={newApp.volumes} onChange={(e) => setNewApp({ ...newApp, volumes: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none" /><p className="text-[10px] font-medium text-gray-400 mt-1.5">source:/container/path[:ro], comma-separated.</p></div>
                <div><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Environment Variables</label><textarea placeholder="NODE_ENV=production&#10;DB_PASSWORD=secret" value={newApp.env_vars} onChange={(e) => setNewApp({ ...newApp, env_vars: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 font-mono h-24 focus:border-[#007aff] focus:outline-none" /><p className="text-[10px] font-medium text-gray-400 mt-1.5">One KEY=VALUE per line.</p></div>
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">CPU Limit (cores)</label><input type="text" placeholder="e.g. 0.5" value={newApp.cpu_limit} onChange={(e) => setNewApp({ ...newApp, cpu_limit: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none" /></div>
                  <div><label className="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">Memory Limit</label><input type="text" placeholder="e.g. 512m, 1g" value={newApp.mem_limit} onChange={(e) => setNewApp({ ...newApp, mem_limit: e.target.value })} className="w-full bg-neutral-50 border border-neutral-200 rounded-[10px] p-3 text-sm text-gray-900 focus:border-[#007aff] focus:outline-none" /></div>
                </div>
              </div>
