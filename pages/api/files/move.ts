...
async function collectFiles(source: string): Promise<{ path: string; relative: string; size: number }[]> {
  ...
  if (!s.isDirectory()) {
    return [{ path: source, relative: path.basename(source), size: s.size }];
  }
...
export default withAuth(async function handler(req: any, res: any) {
  ...
  try {
    ...
    try {
      await mkdir(path.dirname(destinationFullPath), { recursive: true });
      await rename(sourceFullPath, destinationFullPath);
      writeEvent(res, { type: 'progress', progress: 100, bytesMoved: sourceStat.size, totalBytes: sourceStat.size });
    ...
    const emitProgress = (force = false) => {
      ...
      lastEmit = now;
      const progress = totalBytes > 0 ? Math.min(99, Math.round((bytesMoved / totalBytes) * 100)) : 99;
      writeEvent(res, { type: 'progress', progress, bytesMoved, totalBytes });
    };
    ...
    console.error('[/api/files/move]', err);
    writeEvent(res, { type: 'error', error: 'Move failed' });
    res.end();
  }
