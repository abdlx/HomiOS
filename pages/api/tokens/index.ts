...
export default withAuth(async (req, res, session) => {
  ...
  if (req.method === 'POST') {
    ...
    db.prepare('INSERT INTO api_tokens (id, user_id, team_id, name, token_hash, abilities) VALUES (?, ?, ?, ?, ?, ?)')
  ...
  if (req.method === 'DELETE') {
    const { id } = req.body || {};
    const r = db.prepare('DELETE FROM api_tokens WHERE id = ? AND user_id = ?').run(String(id), session.userId);
