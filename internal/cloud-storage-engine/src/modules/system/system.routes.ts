  ...
  exec('git --version', (gitError) => {
    if (gitError) {
      return res.status(400).json({
        code: 'GIT_NOT_FOUND',
        message: 'Git is unavailable. Update the cloud storage subsystem through HomiOS system updates.'
      })
    }
...
systemRouter.get('/backup', requireAuth, (req, res, next) => {
  try {
    ...
    res.setHeader('Content-Disposition', 'attachment; filename=homios-cloud-backup.db')
