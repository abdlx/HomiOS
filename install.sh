...
if [ -f "$PREVIOUS_ENV_FILE" ]; then
  ...
  [ -n "$COOLIFY_VERSION" ] || \
    COOLIFY_VERSION=$(sed -n 's/^COOLIFY_VERSION=//p' "$PREVIOUS_ENV_FILE" | tail -n 1)
  [ -n "$COOLIFY_AUTOUPDATE" ] || \
    COOLIFY_AUTOUPDATE=$(sed -n 's/^COOLIFY_AUTOUPDATE=//p' "$PREVIOUS_ENV_FILE" | tail -n 1)
  [ -n "$HOMIOS_PROXY_MODE" ] || \
...
case "$COOLIFY_MODE" in
  managed)
    ...
    COOLIFY_APP_PORT="$COOLIFY_APP_PORT" COOLIFY_DATA_DIR="$COOLIFY_DATA_DIR" \
      COOLIFY_VERSION="$COOLIFY_VERSION" COOLIFY_AUTOUPDATE="$COOLIFY_AUTOUPDATE" \
...
COOLIFY_VERSION=\$(read_setting COOLIFY_VERSION "$COOLIFY_VERSION")
COOLIFY_AUTOUPDATE=\$(read_setting COOLIFY_AUTOUPDATE "$COOLIFY_AUTOUPDATE")
...
printf 'APP_KEY=%s\nHOMIOS_SAMBA_ALLOWED_ROOTS=%s\nHOMIOS_PORT=%s\nHOMIOS_CONFIG_VERSION=2\nCOOLIFY_MODE=%s\nCOOLIFY_OWNED_BY_HOMIOS=%s\nCOOLIFY_INTEGRATION_ENABLED=%s\nCOOLIFY_ENABLED=%s\nCOOLIFY_APP_PORT=%s\nCOOLIFY_DATA_DIR=%s\nCOOLIFY_VERSION=%s\nCOOLIFY_AUTOUPDATE=%s\nHOMIOS_PROXY_MODE=%s\nCODE_SERVER_ENABLED=true\nCODE_SERVER_PORT=8080\nHOMIOS_BIND_HOST=%s\n' \
  ...
  "\$COOLIFY_APP_PORT" "\$COOLIFY_DATA_DIR" "\$COOLIFY_VERSION" "\$COOLIFY_AUTOUPDATE" \
  "\$HOMIOS_PROXY_MODE" "\$HOMIOS_BIND_HOST" \
  > "\$ENV_FILE"
chmod 600 "\$ENV_FILE"
umask 022
